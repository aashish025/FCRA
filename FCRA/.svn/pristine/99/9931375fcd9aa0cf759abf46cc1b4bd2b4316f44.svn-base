				...
				if (options.validateOnBlur) {
					input
						...
						.on('blur.xdsoft', function () {
							...
							if((inputVal.match(/_/g)||[]).length > 0){
								$(this).val("");
							}
							if (options.allowBlank && !$.trim($(this).val()).length) {
							//if (options.allowBlank && $.trim($(this).val()).match('_').length>0) {
								$(this).val();
			...
			input
				...
				.on('open.xdsoft focusin.xdsoft mousedown.xdsoft touchstart', function (event) {
					...
					timer = setTimeout(function () {
						...
						//--------------------------
						var node = (typeof input[0] === "string" || input[0] instanceof String) ? document.getElementById(input[0]) : input[0];
						if (node) {
							if (node.createTextRange) {
								var textRange = node.createTextRange();
								textRange.collapse(true);
