...
public class Covid19EmergencyRespondReportGenerator extends MISReportGenerator{
	...
	public Covid19EmergencyRespondReportGenerator(Connection connection) {
		super(connection);
	...
	protected void generateExcel() throws Exception {
		 ...
		 String stList=selectStateList.toString().replace("[", "'").replace("]", "'").replace(", ", "','").trim();
          ...
          /*  if(!selectedReligionList.trim().equals("'ALL'")){
            ...
            StateDao sdao= new StateDao(connection);
				...
				reportQuery = ("SELECT   re.rcn,   re.ASSO_NAME,re.ASSO_CHIEF_FUNCTIONARY_NAME,re.EMAIL_ID,   (select ex.ASSO_MOBILE from FC_COVID_RESPONSE_EXEMPTION ex where ex.rcn=re.rcn) mobile,   re.ESTIMATED_AMOUNT_SPENT,re.duration,re.SUPPORT_DESCRIPTION,re.PLACE_ACTION_RESPONSE,   to_char(re.LAST_UPDATED_ON,'dd-mm-yyyy') LAST_UPDATED_ON,   (select   case when   fi.ASSO_ADDRESS is null   then fi.ADD1||','||fi.add2||','||fi.ADD3||fi.ASSO_TOWN_CITY||' Pin Code- '||fi.ASSO_PIN   else   fi.ASSO_ADDRESS||fi.ASSO_TOWN_CITY||' Pin Code- '||fi.ASSO_PIN   end   from FC_INDIA fi where fi.rcn=re.rcn)   address,   (SELECT distname FROM TM_DISTRICT WHERE distcode = SUBSTR(re.stdist,3,5)   )distname, (SELECT sname FROM TM_STATE WHERE scode = SUBSTR(re.stdist,1,2)  )stateName FROM FC_COVID_RESPONSE_ENTRY re where "+StateList+" order by re.ESTIMATED_AMOUNT_SPENT desc");
		...
		String tsPath ="/Reports/Covid19EmergencyRespondedExcel.jrxml";
		String fileName ="Covid19EmergencyRespondedExcel";
	...
	protected void generateHTML() throws Exception {
		...
		covid19Emgresreport=getCovid19EmergencyStateWiseReportHtml();	
	...
	public List<Covid19EmergencyRespondReport> getCovid19EmergencyStateWiseReportHtml() throws Exception {
 		if(connection == null) {
            ...
            /*   if(!selectedReligionList.trim().equals("'ALL'")){
            	   ReligionList= "rname in  ("+selectedReligionList+")";
								...
								StringBuffer query; 
								 ...
								 query = new StringBuffer("SELECT   re.rcn,   re.ASSO_NAME,re.ASSO_CHIEF_FUNCTIONARY_NAME,re.EMAIL_ID,   "
								 		+ "(select ex.ASSO_MOBILE from FC_COVID_RESPONSE_EXEMPTION ex where ex.rcn=re.rcn) mobile, "
								 		+ "  re.ESTIMATED_AMOUNT_SPENT,re.duration,re.SUPPORT_DESCRIPTION,re.PLACE_ACTION_RESPONSE, "
								 		+ "  to_char(re.LAST_UPDATED_ON,'dd-mm-yyyy') LAST_UPDATED_ON,   (select   case when   fi.ASSO_ADDRESS is null "
								 		+ "  then fi.ADD1||','||fi.add2||','||fi.ADD3||fi.ASSO_TOWN_CITY||' Pin Code- '||fi.ASSO_PIN   else "
								 		+ "  fi.ASSO_ADDRESS||fi.ASSO_TOWN_CITY||' Pin Code- '||fi.ASSO_PIN   end   from FC_INDIA fi where fi.rcn=re.rcn)   address, "
								 		+ "  (SELECT distname FROM TM_DISTRICT WHERE distcode = SUBSTR(re.stdist,3,5)   )distname, (SELECT sname FROM TM_STATE WHERE"
								 		+ " scode = SUBSTR(re.stdist,1,2)  )stateName FROM FC_COVID_RESPONSE_ENTRY re where "+StateList+"  order by re.ESTIMATED_AMOUNT_SPENT desc"
								 		);
						  ...
						  System.out.println("query---"+query);
								...
								List<Covid19EmergencyRespondReport> covid19EmergencyRespondReportList = new ArrayList<Covid19EmergencyRespondReport>();
								while(rs.next()) {
									covid19EmergencyRespondReportList.add(new Covid19EmergencyRespondReport(rs.getString(14),rs.getString(1), rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getString(8),rs.getString(9),rs.getString(10),rs.getString(11),rs.getString(12),rs.getString(13))); 
									
								}
								System.out.println("covid19Emgstwisereport----- "+covid19EmergencyRespondReportList);
							return covid19EmergencyRespondReportList;
	 ...
	 @Override
		protected void generatePDF() throws Exception {
			...
			String tsPath = "/Reports/Covid19EmergencyResponded.jrxml";
			String fileName = "Covid19EmergencyResponded.pdf";
	...
	protected void generateCSV() throws Exception {
		...
		String tsPath ="/Reports/Covid19EmergencyRespondedCsv.jrxml";
		String fileName ="Covid19EmergencyResponded";
