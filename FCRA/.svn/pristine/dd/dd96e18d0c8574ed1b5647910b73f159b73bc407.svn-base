...
public class ApplicationStatusDao extends BaseDao<ProjectRequest, String, String>{
	...
	public void cancelApplicationDetails(String appId,String remark,String reason,String user,String reqDate,String type,String officeCode) throws Exception{
		PreparedStatement statement=null;	StringBuffer query =null;
		// Updating FC_INDIA for Registration Cancellation
		query = new StringBuffer("UPDATE FC_INDIA SET CURRENT_STATUS=?,CANCEL_STATUS='Y',CANCEL_DATE=sysdate,CANCEL_REASON=?,CANCEL_USER=?,CANCEL_REMARKS=?"
				+ ",CANCEL_REQ_DATE=to_date(?,'dd-mm-yyyy'),CANCEL_TYPE=? WHERE RCN=?");		
		statement = connection.prepareStatement(query.toString());		
		statement.setString(1, "1");		
		statement.setString(2, reason);
		statement.setString(3, user);
		statement.setString(4, remark);
		statement.setString(5, reqDate);
		statement.setString(6, type);
		statement.setString(7, appId);
		statement.executeUpdate();
		statement.close();
		
		String referenceNumber=generateChatId(officeCode);	// Generating Reference Number
		...
		query=new StringBuffer("INSERT INTO T_REGN_CANCELLATION_DETAILS(REFERENCE_FOR_DETAILS,CANCELLATION_TYPE,CANCELLATION_REASON,CANCELLATION_REQUESTED_DATE) "
				+ "VALUES(?,?,?,to_date(?,'dd-mm-yyyy'))");
		statement = connection.prepareStatement(query.toString());		
		statement.setString(1, referenceNumber);		
		statement.setString(2, type);
		statement.setString(3, reason);		
		statement.setString(4,reqDate);				
		statement.executeUpdate();
		statement.close();
		...
		redrcn.addToRedFlagList(appId, remark, user, "1", null, null, null, "1", officeCode);
		}	
		//connection.commit();
	...
	public void revokeApplicationDetails(String appId,String remark,String user,String officeCode) throws Exception{
		PreparedStatement statement=null;	StringBuffer query =null;
		query = new StringBuffer("UPDATE FC_INDIA SET CURRENT_STATUS=?,CANCEL_STATUS='N',REVOKE_USER=?,REVOKE_DATE=sysdate,REVOKE_REASON=? WHERE RCN=?");		
		statement = connection.prepareStatement(query.toString());		
