...
public class RedFlaggedRcnsDao extends BaseDao<RedFlaggedRcns, String, String> {
	public RedFlaggedRcnsDao(Connection connection) {
		super(connection);
	...
	public List<RedFlaggedRcns> getList() throws Exception{
		 if(connection == null) {
		 ...
		 StringBuffer query = new StringBuffer("  with temp as( SELECT  "
		 		...
		 		+ " to_char(C.STATUS_DATE,'dd-mm-yyyy HH24:MI:SS'),C.ACTION_BY||'('|| e.user_name||')' ,"
		 		+ "  (select CATEGORY_DESC from tm_red_flag_category where CATEGORY_CODE=c.red_flag_category), (case when c.FLAG_TYPE ='2' then 'Yellow' ELSE case when c.FLAG_TYPE='1' then 'Red' END END )  as catogery_type ,c.REMARKS "
		...
		/* StringBuffer query1= new StringBuffer(" with temp as( SELECT  (row_number() over (partition by a.rcn order by  C.STATUS_DATE desc)) as rn ,"
		 		+ "A.RCN,B.ASSO_NAME,(select sname from tm_state where scode=substr(b.stdist,1,2)), "
		 		+ "(select distname from tm_district where distcode=substr(b.stdist,-3,3)),"
		 		+ " to_char(C.STATUS_DATE,'dd-mm-yyyy HH24:MI:SS'),C.ACTION_BY||'('|| e.user_name||')' , "
		 		+ "(select CATEGORY_DESC from tm_red_flag_category where CATEGORY_CODE=c.red_flag_category) , (case when c.FLAG_TYPE ='2' then 'Yellow' ELSE case when c.FLAG_TYPE='1' then 'Red' END END )  as catogery_type "
		 		+ "FROM T_RED_FLAGGED_ASSOCIATIONS A, fc_india B, t_red_flag_status_history C  "
		 		+ " ,TM_User e WHERE c.status=0 AND   A.RCN=B.RCN AND A.RCN=C.RCN AND c.action_by=E.USER_id ),"
		 		+ "T1 AS(select * from temp where rn=1 ),T2 AS (SELECT T1.*, ROWNUM RN1 FROM T1 WHERE ROWNUM<=?)"
		 ...
		 StringBuffer	 query1 = new StringBuffer(" WITH temp AS  (SELECT (row_number() over (partition BY a.rcn order by C.STATUS_DATE DESC)) AS rn ,   A.RCN,    B.ASSO_NAME,  "
					...
					+ " TO_CHAR(C.STATUS_DATE,'dd-mm-yyyy HH24:MI:SS') as statusdate, C.ACTION_BY ||'('|| e.user_name  ||')' ,"
					+ " (SELECT CATEGORY_DESC  FROM tm_red_flag_category  WHERE CATEGORY_CODE=c.red_flag_category), "
					+ "( CASE  WHEN c.FLAG_TYPE ='2' THEN 'Yellow' ELSE  CASE WHEN c.FLAG_TYPE='1'  THEN 'Red' END END ) AS catogery_type, C.STATUS_DATE as statusdate1, c.REMARKS"
					+ "  FROM T_RED_FLAGGED_ASSOCIATIONS A,  fc_india B, t_red_flag_status_history C ,  TM_User e  WHERE c.status =0  AND"
					+ " A.RCN   =B.RCN  AND A.RCN      =C.RCN  AND c.action_by=E.USER_id ");
				/*	+ ",t1 as(SELECT * FROM temp WHERE rn=1  ),"
		...
		statement.close();
		 ...
		 List<RedFlaggedRcns> redflaggedList = new ArrayList<RedFlaggedRcns>();
		 while(rs.next()) {	
		
			 redflaggedList.add(new RedFlaggedRcns(rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getString(8),rs.getString(9),rs.getString(11)));	
		 }
		 return redflaggedList;
		 }
	
	// Annual Model Open Query 
	public List<RedFlaggedRcns> getAnnualList() throws Exception{
		 if(connection == null) {
		 throw new Exception("Invalid connection");
		 }
	
		 StringBuffer query = new StringBuffer(" WITH temp AS (SELECT (row_number() over (partition BY a.rcn order by C.STATUS_DATE DESC)) AS rn , A.RCN,  B.ASSO_NAME, "
		 		+ "(SELECT sname FROM tm_state WHERE scode=SUBSTR(b.stdist,1,2)) AS state, (SELECT distname FROM tm_district WHERE distcode=SUBSTR(b.stdist,-3,3) ),"
		 		+ " TO_CHAR(C.STATUS_DATE,'dd-mm-yyyy HH24:MI:SS') AS statusdate, ( SELECT DISTINCT c.ACTION_BY ||'('|| e.user_name ||')' FROM TM_User E WHERE c.action_by=E.USER_id) AS action ,"
		 		+ " (SELECT CATEGORY_DESC   FROM tm_red_flag_category   WHERE CATEGORY_CODE=c.red_flag_category  ), c.red_flag_category,( CASE  WHEN c.FLAG_TYPE ='2' THEN 'Yellow'  ELSE   CASE"
		 		+ " WHEN c.FLAG_TYPE='1'   THEN 'Red'  END  END )         AS catogery_type, C.STATUS_DATE AS statusdate1,  c.REMARKS  FROM T_RED_FLAGGED_ASSOCIATIONS A,    fc_india B,  t_red_flag_status_history C ,"
		 		+ "    TM_User e WHERE c.status =0  AND A.RCN      =B.RCN  AND A.RCN      =C.RCN  ORDER BY statusdate1 DESC  ),  t1 AS  (SELECT * FROM temp WHERE rn=1  ),  t2 AS  ( SELECT * FROM T1 WHERE red_flag_category=9  ) ,"
		 		+ "  t3 AS  (SELECT i.rcn,    i.asso_name,    (SELECT sname FROM tm_state WHERE scode=SUBSTR(stdist,1,2)    ) state_name,    (SELECT DISTNAME    FROM tm_district    WHERE scode =SUBSTR(stdist,1,2) "
		 		+ "   AND DISTCODE=SUBSTR(stdist,-3,3)    ) district_name,    NVL(TO_CHAR(reg_date,'dd-mm-yyyy'),'---')   AS REGISTRERED_ON,    NVL(TO_CHAR(LAST_RENEWED_ON,'dd-mm-yyyy'),'---') AS LAST_RENEWED_ON,  "
		 		+ "  NVL(TO_CHAR(valid_to,'dd-mm-yyyy'),'---')        AS EXPIRY_ON,    CASE   WHEN reg_date IS NULL THEN 2006  ELSE   "
		 		+ "   CASE WHEN to_number(TO_CHAR(reg_date,'yyyy'))<2006  THEN 2006  ELSE CASE WHEN TRUNC(reg_date) >= to_date('01-APR-'||TO_CHAR(reg_date,'yyyy')) THEN to_number(TO_CHAR(reg_date,'yyyy'))+1"
		 		+ "    ELSE to_number(TO_CHAR(reg_date,'yyyy')) END END END start_year, to_number(TO_CHAR(sysdate, 'YYYY'))-1 end_year FROM fc_india i,  t2 WHERE i.rcn=t2.rcn ), t4 AS"
		 		+ "( SELECT t3.*,(end_year-start_year) to_be_uploaded FROM t3 ) , t5 AS (SELECT a.rcn, COUNT(blkyear) AS uploaded FROM t4,  fc_fc3_tally a WHERE t4.rcn=a.rcn"
		 		+ "  AND to_number(SUBSTR(blkyear,-4,4)) BETWEEN start_year AND end_year  GROUP BY a.rcn  ),  t6 AS  (SELECT t4.*,NVL(t5.uploaded,0) uploaded  FROM t4 LEFT JOIN t5"
		 		+ "  ON t5.rcn=t4.rcn)SELECT * FROM t6 WHERE to_be_uploaded<=uploaded  ");
		...
		statement.close();
		 ...
		 Integer pageRequested = Integer.parseInt(pageNum);
		 Integer pageSize = Integer.parseInt(recordsPerPage);
		 String queryForPaging1 = preparePagingQuery2(query);	
        ...
        if(pageNum == null || recordsPerPage == null) {
		 ...
		 while(rs.next()) {	
			 ...
			 redflaggedList.add(new RedFlaggedRcns(rs.getString(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getString(8),rs.getString(9),rs.getString(10),rs.getString(11)));	
		 }
		 return redflaggedList;
		 }
	...
	// End 
	public List<RedFlaggedRcns> getAnnualStatusList(String annualrcn) throws Exception{
		 if(connection == null) {
		 throw new Exception("Invalid connection");
		 }
		 StringBuffer query = new StringBuffer("with t1 as (select (case when reg_date is null then 2005 else case when trunc(reg_date) >= to_date('01-APR-'||to_char(reg_date, 'YYYY')) then to_number(to_char(reg_date, 'YYYY'))-1 "
			...
			ResultSet rs = statement.executeQuery();
			 List<RedFlaggedRcns> yearList = new ArrayList<RedFlaggedRcns>();
			 StringBuffer query1; 
			 while(rs.next()) {	
				...
				// status = "No";
				 query1 = new StringBuffer("select (Case when trunc(reg_date) >= to_date('01-APR-'||substr(?,-4,4)) then '-' else "
					...
					if(rs1.next()){
						 yearList.add(new RedFlaggedRcns(rs.getString(1), rs1.getString(1)));	
					}
	 ...
	 public void removeFromRedFlagList(String appId,String remark,String user,String office,String order,String date,String myOfficeCode) throws Exception{
			...
			query = new StringBuffer("INSERT INTO T_RED_FLAG_STATUS_HISTORY(RCN,STATUS,REMARKS,STATUS_DATE,ACTION_BY,ORIGINATOR_OFFICE,ORIGINATOR_ORDER_NO,ORIGINATOR_ORDER_DATE,RED_FLAG_CATEGORY,FLAG_TYPE,ACTION_BY_OFFICE) "
					+ " VALUES(?,?,?,systimestamp,?,?,?,to_date(?,'dd-mm-yyyy'),null,null,?)");
			statement = connection.prepareStatement(query.toString());		
			statement.setString(1, appId);
			...
			statement.setString(8,myOfficeCode);
			statement.executeUpdate();		
			//connection.commit();
			statement.close();
	...
	private String preparePagingQuery2(StringBuffer query) throws Exception {
		...
		if(pageNum == null || recordsPerPage == null)
			...
			queryForPaging = new StringBuffer(" WITH temp AS (SELECT (row_number() over (partition BY a.rcn order by C.STATUS_DATE DESC)) AS rn , A.RCN,  B.ASSO_NAME, "
			 		+ "(SELECT sname FROM tm_state WHERE scode=SUBSTR(b.stdist,1,2)) AS state, (SELECT distname FROM tm_district WHERE distcode=SUBSTR(b.stdist,-3,3) ),"
			 		+ " TO_CHAR(C.STATUS_DATE,'dd-mm-yyyy HH24:MI:SS') AS statusdate, ( SELECT DISTINCT c.ACTION_BY ||'('|| e.user_name ||')' FROM TM_User E WHERE c.action_by=E.USER_id) AS action ,"
			 		+ " (SELECT CATEGORY_DESC   FROM tm_red_flag_category   WHERE CATEGORY_CODE=c.red_flag_category  ), c.red_flag_category,( CASE  WHEN c.FLAG_TYPE ='2' THEN 'Yellow'  ELSE   CASE"
			 		+ " WHEN c.FLAG_TYPE='1'   THEN 'Red'  END  END )         AS catogery_type, C.STATUS_DATE AS statusdate1,  c.REMARKS  FROM T_RED_FLAGGED_ASSOCIATIONS A,    fc_india B,  t_red_flag_status_history C ,"
			 		...
			 		+ "( SELECT t3.*,(end_year-start_year) to_be_uploaded FROM t3 ) , t5 AS (SELECT a.rcn, COUNT(blkyear) AS uploaded FROM t4,  fc_fc3_tally a WHERE t4.rcn=a.rcn"
			 		+ "  AND to_number(SUBSTR(blkyear,-4,4)) BETWEEN start_year AND end_year  GROUP BY a.rcn  ),  t6 AS  (SELECT t4.*,NVL(t5.uploaded,0) uploaded  FROM t4 LEFT JOIN t5"
			 		+ "  ON t5.rcn=t4.rcn),  t7 as (SELECT * FROM t6 WHERE to_be_uploaded<=uploaded "+orderbyClause+" ) ,T8 AS (SELECT T7.*, ROWNUM RN FROM T7 ) SELECT * FROM T8 WHERE RN BETWEEN ? and ? ");
		return queryForPaging.toString();
	}
	...
	public void addToRedFlagList(String appId,String remark,String user,String category,String office,String order,String date, String flagvalue, String myofficeCode) throws Exception{
		PreparedStatement statement=null;
		int checkflag=0;
		...
		StringBuffer query1 = new StringBuffer("INSERT INTO T_RED_FLAG_STATUS_HISTORY(RCN,STATUS,REMARKS,STATUS_DATE,ACTION_BY,ORIGINATOR_OFFICE,ORIGINATOR_ORDER_NO,ORIGINATOR_ORDER_DATE,RED_FLAG_CATEGORY,FLAG_TYPE,ACTION_BY_OFFICE) "
				+ "VALUES(?,?,?,systimestamp,?,?,?,to_date(?,'dd-mm-yyyy'),?,?,?)");
		...
		statement.setString(9, flagvalue);
		statement.setString(10, myofficeCode);
		statement.executeUpdate();	
		//connection.commit();			
	} 
	public void getApplicationDetails(String appId,String myOfficeCode,String myUserId) throws Exception{		
		...
		// Checking Logged in User having Yellow Flag Add Role
		query=new StringBuffer("SELECT * FROM TM_USER_ROLE WHERE USER_ID=? AND ROLE_ID=15  AND RECORD_STATUS = 0");
		statement = connection.prepareStatement(query.toString());	
		statement.setString(1, myUserId);		
		rs=statement.executeQuery();
		if(rs.next()){
			yellowFlagAddRole="YES";
		}else{
			yellowFlagAddRole="NO";
		}
...
/*		query=new StringBuffer("SELECT APPLICATION_ID,TO_CHAR(SUBMISSION_DATE,'DD-MM-YYYY'),"
		...
		// History --above qury is changed because data was not coming very slow.... I changed qury with with as clause.  
		query=new StringBuffer("with t as (SELECT APPLICATION_ID, TO_CHAR(SUBMISSION_DATE,'DD-MM-YYYY') as SUBMISSION_DATE, (SELECT SERVICE_DESC FROM TM_SERVICE WHERE SERVICE_CODE=V.SERVICE_CODE) SERVICE,(SELECT FILESTATUS_DESC  FROM TM_FILESTATUS  WHERE FILESTATUS_ID=V.CURRENT_STATUS  ) STATUS FROM V_APPLICATION_DETAILS V),t2 as ((SELECT UNIQUE_FILENO   FROM     ( SELECT UNIQUE_FILENO, ASSO_FCRA_RCN RCN FROM FC_FC5_ENTRY_NEW1     UNION     SELECT UNIQUE_FILENO, RCN FROM FC_FC6_FORM     UNION     SELECT UNIQUE_FILENO,       RCN     FROM T_FC8_ENTRY A,       FC_INDIA B     WHERE A.SECTION_FILENO=B.SECTION_FILENO     )   WHERE UPPER(RCN)=UPPER('"+appId+"')   )) select * from t where APPLICATION_ID in (select UNIQUE_FILENO from t2) ORDER BY SUBMISSION_DATE");
	...
	public int findRoleId(String userId) throws SQLException{
		StringBuffer query = new StringBuffer("SELECT  ROLE_ID FROM TM_USER_ROLE where USER_ID=? AND ROLE_ID=16 and RECORD_STATUS=0");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, userId);
		ResultSet rs=statement.executeQuery();
		int addRoleId=0;
		if(rs.next()){
			addRoleId=rs.getInt(1);
		}
	
	  return addRoleId;	
	}
	public int yellowFlagRole(String userId) throws SQLException{
		StringBuffer query = new StringBuffer("SELECT  ROLE_ID FROM TM_USER_ROLE where USER_ID=? AND  ROLE_ID=15 and RECORD_STATUS=0");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, userId);
		ResultSet rs=statement.executeQuery();
		int addRoleId=0;
	...
	public int redremoveFlagRole(String userId) throws SQLException{
		StringBuffer query = new StringBuffer("SELECT  ROLE_ID FROM TM_USER_ROLE where USER_ID=? AND  ROLE_ID=17 and RECORD_STATUS=0");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, userId);
		ResultSet rs=statement.executeQuery();
		int addRoleId=0;
	...
	public int yellowremoveRole(String userId) throws SQLException{
		StringBuffer query = new StringBuffer("SELECT  ROLE_ID FROM TM_USER_ROLE where USER_ID=? AND  ROLE_ID=18 and RECORD_STATUS=0");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, userId);
		ResultSet rs=statement.executeQuery();
		int addRoleId=0;
