 ...
 *
 */
public class ChangeOfFcRecieptCumUtilisationBank extends AbstractRequest {
	...
	public ChangeOfFcRecieptCumUtilisationBank(Connection connection) {
		super(connection);
	}	
	...
	private void retrieveRegistrationDetails() throws Exception {
		...
		StringBuffer query = new StringBuffer("SELECT A.ASSO_NAME, A.ASSO_ADDRESS, A.ASSO_TOWN_CITY, SUBSTR(A.STDIST, 1, 2), "
				...
				+ "FROM FC_INDIA A, FC_FC6_BANK_CHANGE C WHERE A.RCN=C.RCN AND C.UNIQUE_FILENO=?");
	...
	private void retrieveRegisteredRecipientBankDetails() throws Exception {
		...
		StringBuffer query = new StringBuffer("SELECT A.ACCOUNT_NO, A.BANK_NAME, (SELECT B.BANK_NAME FROM TM_BANKS B WHERE to_char(BANK_CODE)=A.BANK_NAME), "
				...
				+ "A.BANK_PIN "
				+ "FROM FC_BANK A, FC_FC6_BANK_CHANGE C WHERE A.RCN=C.RCN AND C.UNIQUE_FILENO=?");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1,applicationId);
		ResultSet rs = statement.executeQuery();
		if(rs.next()) {
...
/*		StringBuffer query = new StringBuffer("SELECT ASSO_NAME_CHANGE_ST, ASSO_CHANGE_STATE_ST, ASSO_CHANGE_DESIGBANK_STATUS, "
		...
		StringBuffer query = new StringBuffer("SELECT CHANGED_ACCOUNT_NO, CHANGED_BANK_NAME, "
				+ "(SELECT B.BANK_NAME FROM TM_BANKS B WHERE to_char(BANK_CODE)=C.CHANGED_BANK_NAME),CHANGED_BANK_ADDRESS, CHANGED_BANK_TOWN_CITY, "
				+ "CHANGED_BANK_STATE, (SELECT SNAME FROM TM_STATE WHERE SCODE=CHANGED_BANK_STATE), CHANGED_BANK_DISTRICT, "
				+ "(SELECT DISTNAME FROM TM_DISTRICT WHERE SCODE=CHANGED_BANK_STATE AND DISTCODE=CHANGED_BANK_DISTRICT), CHANGED_BANK_PIN, "
				+ "CHANGED_BANK_IFSC,CHANGED_BANK_ACCOUNT_DATE,CHANGED_BANK_EMAIL	FROM FC_FC6_BANK_CHANGE C WHERE C.UNIQUE_FILENO=?");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, applicationId);
		ResultSet rs = statement.executeQuery();
		if(rs.next()) {
			...
			bankDetails.setPincode(rs.getString(i++));
			bankDetails.setIfscCode(rs.getString(i++));
			bankDetails.setChangedBankAcctDate(rs.getString(i++));
			bankDetails.setChangedBankMailId(rs.getString(i++));
			changeOfReceipientBank  = "Y";
	...
	private void retrieveUploadedDocumentDetails() throws Exception {
		...
		* */
		...
		StringBuffer query = new StringBuffer("SELECT CHK_CODE, CHK_NAME, CHK_NAME "
					+ "FROM FC_FC6_CHECKLIST_BANK_CHANGE WHERE CHK_CODE NOT IN ('01', '02') AND STATUS='Y'");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		ResultSet rs = statement.executeQuery();
		String docName = null;
		String docDetails = null;
		...
		while(rs.next()) {
			int j = 1;
			String docPath = documentRoot.concat("/").concat(creationDate.substring(6, 10)).concat("/").concat(creationDate.substring(3, 5)).concat("/").concat(creationDate.substring(0, 2)).concat("/15/").concat(documentId).concat("/");
