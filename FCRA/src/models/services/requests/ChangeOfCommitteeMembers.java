...
public class ChangeOfCommitteeMembers extends AbstractRequest {
	...
	public ChangeOfCommitteeMembers(Connection connection) {
		super(connection);
	}	
...
/*		StringBuffer query = new StringBuffer("SELECT A.ASSO_NAME, A.ASSO_ADDRESS, A.ASSO_TOWN_CITY, SUBSTR(A.STDIST, 1, 2), "
				...
				+ "(SELECT RELIGION_DESC FROM TM_RELIGION WHERE RELIGION_CODE=ASSO_RELIGION), A.RCN, TO_CHAR(REG_DATE, 'dd-mm-yyyy'),"
				+ "TO_CHAR(FORM_SUBMISSION_DATE, 'dd-mm-yyyy') "
				+ "FROM FC_INDIA A, FC_FC6_FORM C WHERE A.RCN=C.RCN AND C.UNIQUE_FILENO=?");*/
		StringBuffer query = new StringBuffer("SELECT A.ASSO_NAME, A.ASSO_ADDRESS, A.ASSO_TOWN_CITY, SUBSTR(A.STDIST, 1, 2), "
				+ "(SELECT SNAME FROM TM_STATE WHERE SCODE=SUBSTR(A.STDIST, 1, 2)), SUBSTR(A.STDIST, 3), (SELECT DISTNAME FROM TM_DISTRICT WHERE SCODE=SUBSTR(A.STDIST, 1, 2) AND DISTCODE=SUBSTR(A.STDIST, 3)), "
				+ "A.ASSO_PIN, A.ASSO_NATURE, "
				+ "(SELECT NATURE_DESC FROM TM_NATURE WHERE NATURE_CODE=A.ASSO_NATURE), ASSO_RELIGION, "
				...
				+ "TO_CHAR(FORM_SUBMISSION_DATE, 'dd-mm-yyyy') "
				+ "FROM FC_INDIA A, FC_FC6_MEMBERS_CHANGE_ENTRY C WHERE A.RCN=C.RCN AND C.UNIQUE_FILENO=?");		
...
/*	private void retrieveRegisteredRecipientBankDetails() throws Exception {
	...
	private void retrieveDetailsOfChange() throws Exception {
		...
		StringBuffer query = new StringBuffer("SELECT IS_CHANGE_MEMBERS FROM FC_FC6_MEMBERS_CHANGE_ENTRY C WHERE C.UNIQUE_FILENO=?");		
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, applicationId);
		ResultSet rs = statement.executeQuery();
		if(rs.next()) {
...
/*	private void retrieveChangeInUtilizationBanks() throws Exception {
	...
	private void retrieveChangeInCommitteeMemberDetails() throws Exception {
		...
		StringBuffer query = new StringBuffer("SELECT NAME, FATHER_HUSBAND_NAME, NATIONALITY, "
				...
				+ "FROM FC_FC6_MEMBERS WHERE UNIQUE_FILENO=?");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, applicationId);
		ResultSet rs = statement.executeQuery();
		while(rs.next()) {
	...
	private void retrieveUploadedDocumentDetails() throws Exception {
		...
		* */
		...
		StringBuffer query = new StringBuffer("SELECT CHK_CODE, CHK_NAME, CHK_NAME "
					+ "FROM FC_FC6_CHECKLIST_MEMBER_CHANGE WHERE CHK_CODE NOT IN ('01', '02') AND STATUS='Y'");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		ResultSet rs = statement.executeQuery();
		String docName = null;
		String docDetails = null;
		...
		while(rs.next()) {
			int j = 1;
			String docPath = documentRoot.concat("/").concat(creationDate.substring(6, 10)).concat("/").concat(creationDate.substring(3, 5)).concat("/").concat(creationDate.substring(0, 2)).concat("/17/").concat(documentId).concat("/");
...
private void retrievedeletedDocumentDetails() throws Exception {
	...
	private void retrieveaffidavitDocumentDetails() throws Exception {
		...
		StringBuffer query = new StringBuffer("select FILENO,SL_NO,name,FATHER_HUSBAND_NAME,(select desig_name from TM_COMMITTEE_DESIGNATION where OFFICE_OF_ASSO=desig_code) from FC_FC6_COMMITTEE where UNIQUE_FILENO=?");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, applicationId);
		ResultSet rs = statement.executeQuery();
