...
public class ChangeOfOpeningUtilisationBankAccount extends AbstractRequest {
	...
	public ChangeOfOpeningUtilisationBankAccount(Connection connection) {
		super(connection);
	}	
	...
	private void retrieveRegistrationDetails() throws Exception {
		...
		StringBuffer query = new StringBuffer("SELECT A.ASSO_NAME, A.ASSO_ADDRESS, A.ASSO_TOWN_CITY, SUBSTR(A.STDIST, 1, 2), "
				...
				+ "(SELECT RELIGION_DESC FROM TM_RELIGION WHERE RELIGION_CODE=ASSO_RELIGION), A.RCN, TO_CHAR(C.REG_DATE, 'dd-mm-yyyy'),"
				+ "TO_CHAR(FORM_SUBMISSION_DATE, 'dd-mm-yyyy') "
				+ "FROM FC_INDIA A, FC_FC6_UTILIZATION_ACCTS_ENTRY C WHERE A.RCN=C.RCN AND C.UNIQUE_FILENO=?");
...
/*	private void retrieveRegisteredRecipientBankDetails() throws Exception {
	...
	private void retrieveDetailsOfChange() throws Exception {
		...
		StringBuffer query = new StringBuffer("SELECT NEW_ACCOUNT_NO,NEW_BANK_NAME,(SELECT B.BANK_NAME FROM TM_BANKS B WHERE TO_CHAR(BANK_CODE)=C.NEW_BANK_NAME),"
				+ " NEW_BANK_ADDRESS,NEW_BANK_TOWN_CITY,NEW_BANK_STATE,"
				+ "(SELECT SNAME FROM TM_STATE WHERE SCODE=NEW_BANK_STATE),NEW_BANK_DISTRICT,"
				+ " (SELECT DISTNAME FROM TM_DISTRICT WHERE SCODE =NEW_BANK_STATE AND DISTCODE=NEW_BANK_DISTRICT),NEW_BANK_PIN,NEW_BANK_IFSC "
				+ " FROM FC_FC6_NEW_UTILIZATION_ACCTS C WHERE C.UNIQUE_FILENO=?");
		//String query = new StringBuffer("SELECT NEW_ACCOUNT_NO,NEW_BANK_NAME,(SELECT B.BANK_NAME FROM TM_BANKS B WHERE TO_CHAR(BANK_CODE)=C.NEW_BANK_NAME),NEW_BANK_ADDRESS,NEW_BANK_TOWN_CITY,NEW_BANK_STATE,(SELECT SNAME FROM TM_STATE WHERE SCODE=NEW_BANK_STATE),NEW_BANK_DISTRICT,(SELECT DISTNAME FROM TM_DISTRICT WHERE SCODE =NEW_BANK_STATE AND DISTCODE=NEW_BANK_DISTRICT),NEW_BANK_PIN,NEW_BANK_IFSC FROM FC_FC6_NEW_UTILIZATION_ACCTS C WHERE C.UNIQUE_FILENO=?");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, applicationId);
		ResultSet rs = statement.executeQuery();
		if(rs.next()) {
...
/*			changeOfNameOrAim = rs.getString(i++);
			changeOfAddress = rs.getString(i++);
	...
	private void retrieveUploadedDocumentDetails() throws Exception {
		...
		* */
		...
		StringBuffer query = new StringBuffer("SELECT CHK_CODE, CHK_NAME, CHK_NAME "
					+ "FROM FC_FC6_CHECKLIST_UTILISANACCT WHERE CHK_CODE NOT IN ('01', '02') AND STATUS='Y'");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		ResultSet rs = statement.executeQuery();
		String docName = null;
		String docDetails = null;
		...
		while(rs.next()) {
			int j = 1;
			String docPath = documentRoot.concat("/").concat(creationDate.substring(6, 10)).concat("/").concat(creationDate.substring(3, 5)).concat("/").concat(creationDate.substring(0, 2)).concat("/16/").concat(documentId).concat("/");
