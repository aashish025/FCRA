...
public class PriorPermission extends AbstractRequest {
	...
	private String natureOfContribution;
	private String natureOfContributionDesc;
	...
	public PriorPermission(Connection connection) {
		super(connection);
	}	
	...
	private void retrieveRegistrationDetails() throws Exception {
		...
		StringBuffer query = new StringBuffer("SELECT APPLICANT_NAME, ASSO_NAME, ASSO_ADDRESS, ASSO_TOWN_CITY, ASSO_STATE, "
				...
				+ "ASSO_RELIGION_OTHER, MAIN_AIMS, ACCOUNT_NO, BANK_NAME, (SELECT B.BANK_NAME FROM TM_BANKS B WHERE to_char(BANK_CODE)=A.BANK_NAME), "
				...
				+ "BANK_PIN, BANK_IFSC_CODE, CONVITECTED_BY_COURT, PROSECUTION_OFFENCE_PENDING, GUILTY_DIVRSN_MISUTILISN_FUND, "
				+ "PROHIBITED_FORGN_CONTIBUTION, CHEIF_OFFICE_BEAR_YES_NO, MEMBER_OF_ANOTHER_ASSO, CHEIF_PARTRON_DETAIL, "
				+ "BUNIT_ASSO_ALREDY_REG_YES_NO, BUNIT_ASSO_ALREDY_REG_NAME, BUNIT_ASSO_ALREDY_REG_ADDR, asso_reg_no, to_char(ASSO_REG_DATE, 'dd-mm-yyyy'), "
				+ "DIRECTED_BY_CG_SEC1_YES_NO, DIRECTED_BY_CG_SEC1_ORDERNO, to_char(DIRECTED_BY_CG_SEC1_ORDERDATE, 'dd-mm-yyyy'), DIRECTED_BY_CG_SEC10_YES_NO, "
				+ "DIRECTED_BY_CG_SEC10_ORDERNO, to_char(DIRECTED_BY_CG_SEC10_ORDERDT, 'dd-mm-yyyy'), EARLIER_PROCEEDED_YES_NO, EARLIER_PROCEEDED_DETAILS, "
				+ "PERMISSION_GRANTED_YES_NO, PERMISSION_GRANTED_NUMBER, to_char(PERMISSION_GRANTED_DATE, 'dd-mm-yyyy'), DATE_OF_ACCOUNT_SUBMN_YES_NO, "
				+ "to_char(DATE_OF_ACCOUNT_SUBMN, 'dd-mm-yyyy'), RECVD_FORGN_VIOLATION_YES_NO, CURRENCY, (SELECT CURR_NAME FROM TM_CURRENCY WHERE CURR_CODE=CURRENCY), "
				...
				+ "PROHIBITD_FRM_FORGN_CONTRBN, PRHBTD_FRM_FRN_CONTRBN_DTLS, EVER_APPLD_ASSO_YES_NO, EVER_APPLIED_REG_NUMBER, "
				+ "EVER_APPLD_DATE_OF_APP_SUBMSN, EVER_APPLD_LAST_REF_NO, to_char(EVER_APPLD_LAST_REF_DATE, 'dd-mm-yyyy'), EVER_APPLD_REG_REFUSED_YES_NO, EVER_APPLD_REG_PENDING_YES_NO, "
				+ "EVER_APPLIED_PP_YES_NO, EVER_APPLIED_PP_NUMBER, to_char(EVER_APPLIED_PP_DATE, 'dd-mm-yyyy'), EVER_APPLIED_LAST_PP_NO, "
				+ "to_char(EVER_APPLIED_LAST_PP_DATE, 'dd-mm-yyyy'), EVER_APPLIED_PP_REFSD_YES_NO, EVER_APPLIED_PP_PENDG_YES_NO, ASSO_CLSD_LNK_REG_REFD_YES_NO, "
				+ "ASSO_CLSD_LNK_REG_PRHD_YES_NO, ASSO_CLSD_LNK_OTH_ASSO_YES_NO, ASSO_CLSD_LNK_OTHER_ASSO_NAME, ASSO_CLSD_LNK_OTHER_ASSO_ADDR, TO_CHAR(FORM_SUBMISSION_DATE, 'dd-mm-yyyy'),DARPANID,DARPAN_PANNO "
				+ "FROM FC_FC1A_ENTRY A WHERE UNIQUE_FILENO=?");
	...
	private void retrieveDonorDetails() throws Exception {
		...
		StringBuffer query = new StringBuffer("SELECT NATUREANDVALUE_YES_NO, "
				...
				+ "FROM FC_FC1A_DONOR WHERE UNIQUE_FILENO=?");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, applicationId);
		ResultSet rs = statement.executeQuery();
		while(rs.next()) {
	...
	private void retrieveCommitteeMemberDetails() throws Exception {
		...
		StringBuffer query = new StringBuffer("SELECT NAME, FATHER_HUSBAND_NAME, NATIONALITY, "
				...
				+ "FROM FC_FC1A_COMMITTEE_NEW a LEFT JOIN FC_FC1A_FOREIGNERS b  ON a.FILENO=b.FILENO AND a.SL_NO=b.SL_NO WHERE a.UNIQUE_FILENO=?");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, applicationId);
		ResultSet rs = statement.executeQuery();
		while(rs.next()) {
			...
			member.setRelationWithOfficeBearersOther(rs.getString(14));
	...
	private void retrieveUploadedDocumentDetails() throws Exception {
		...
		StringBuffer query = new StringBuffer("SELECT UPLOADED_DOC, TO_CHAR(SUBMIT_DATE, 'dd-mm-yyyy') "
				+ "FROM FC_FC1A_UPLOAD_DOC WHERE UNIQUE_FILENO=?");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, applicationId);
		ResultSet rs = statement.executeQuery();
		...
		if(uploadedList != null && uploadedList.equals("") == false) {
			query = new StringBuffer("SELECT CHK_DETAIL, CHK_DESC "
					+ "FROM FC_FC1A_CHECKLIST WHERE CHK_CODE=? AND CHK_CODE NOT IN ('01', '08')");
			...
			for(int i=0; i<uploadedList.length(); i+=2) {
				...
				String docPath = documentRoot.concat("/").concat(creationDate.substring(6, 10)).concat("/").concat(creationDate.substring(3, 5)).concat("/").concat(creationDate.substring(0, 2)).concat("/02/").concat(documentId).concat("/");
				if(rs.next()) {
					int j = 1;
		...
		* */
		...
		StringBuffer query = new StringBuffer("SELECT CHK_CODE, CHK_DETAIL, CHK_DESC "
					+ "FROM FC_FC1A_CHECKLIST WHERE CHK_CODE NOT IN ('01', '08') AND STATUS='Y'");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		ResultSet rs = statement.executeQuery();
		String docName = null;
		String docDetails = null;
	...
	private void retrieveaffidavitDocumentDetails() throws Exception {
		...
		StringBuffer query = new StringBuffer("select FILENO,SL_NO,name,FATHER_HUSBAND_NAME,(select desig_name from TM_COMMITTEE_DESIGNATION where OFFICE_OF_ASSO=desig_code)from FC_FC1A_COMMITTEE_NEW where UNIQUE_FILENO=?");
		PreparedStatement statement = connection.prepareStatement(query.toString());
		statement.setString(1, applicationId);
		ResultSet rs = statement.executeQuery();
	...
	public String getNatureOfContributionDesc() {
		return natureOfContributionDesc;
	}

	public void setNatureOfContributionDesc(String natureOfContributionDesc) {
		this.natureOfContributionDesc = natureOfContributionDesc;
	}
	...
	public String getCurrencyOther() {
		return currencyOther;
	}

	public void setCurrencyOther(String currencyOther) {
